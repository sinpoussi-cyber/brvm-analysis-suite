...
def run_technical_analysis():
    logging.info("="*60)
    logging.info("📈 ÉTAPE 2: ANALYSE TECHNIQUE (V7.0)")
    logging.info("Écriture dans colonnes F-X des feuilles existantes")
    logging.info("="*60)
    ...
    if gc:
        try:
            ...
            logging.warning(f"⚠️  Impossible d'ouvrir le spreadsheet: {e}")
    ...
    try:
        ...
        for company_id, symbol in companies:
            total_updates += process_company(conn, gc, spreadsheet, company_id, symbol)
        
        elapsed = time.time() - start_time
        
        logging.info("\n" + "="*60)
        logging.info("✅ ANALYSE TECHNIQUE TERMINÉE")
        logging.info(f"⏱️  Durée: {elapsed:.2f}s")
